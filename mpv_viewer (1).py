...
class MPVViewer:
    def __init__(self, volume_data, metadata, organ_volume=None, dicom_files=None, dicom_path=None):
        ...
        self.fig.canvas.manager.set_window_title('MPV Viewer - DUAL AI Enhanced')
        ...
        # Cine speed - MOVED UP to avoid overlap
    ...
    def setup_widgets(self):
        ...
        # CHANGED: Cine axis button now shows Axial (instead of Z-axis)
    ...
    def cycle_cine_axis(self, event):
        """Cycle cine axis - CHANGED to show Sagittal/Coronal/Axial"""
        axes = ['z', 'x', 'y']  # z=axial, x=sagittal, y=coronal
        current_idx = axes.index(self.cine_axis)
        self.cine_axis = axes[(current_idx + 1) % len(axes)]
        
        # Map axis to anatomical view name
    ...
    def get_organ_slice(self):
        """Get organ slice with auto-detected orientation"""
        ...
        try:
            if self.organ_view_axis == 'axial':
                ...
                if axis_info is None:
                    return self.organ_volume[min(self.cursor_z, organ_shape[0]-1), :, :]
                
                axis, transpose = axis_info
                
                if axis == 0:
                    slice_data = self.organ_volume[min(self.cursor_z, organ_shape[0]-1), :, :]
                elif axis == 1:
                    slice_data = self.organ_volume[:, min(self.cursor_z, organ_shape[1]-1), :]
                elif axis == 2:
                    slice_data = self.organ_volume[:, :, min(self.cursor_z, organ_shape[2]-1)]
                ...
                if axis_info is None:
                    return self.organ_volume[:, :, min(self.cursor_x, organ_shape[2]-1)]
                
                axis, transpose = axis_info
                
                if axis == 0:
                    slice_data = self.organ_volume[min(self.cursor_x, organ_shape[0]-1), :, :]
                elif axis == 1:
                    slice_data = self.organ_volume[:, min(self.cursor_x, organ_shape[1]-1), :]
                elif axis == 2:
                    slice_data = self.organ_volume[:, :, min(self.cursor_x, organ_shape[2]-1)]
                ...
                if axis_info is None:
                    return self.organ_volume[:, min(self.cursor_y, organ_shape[1]-1), :]
                
                axis, transpose = axis_info
                
                if axis == 0:
                    slice_data = self.organ_volume[min(self.cursor_y, organ_shape[0]-1), :, :]
                elif axis == 1:
                    slice_data = self.organ_volume[:, min(self.cursor_y, organ_shape[1]-1), :]
                elif axis == 2:
                    slice_data = self.organ_volume[:, :, min(self.cursor_y, organ_shape[2]-1)]
    ...
    def update_all_views(self):
        ...
        try:
            ...
            cmap = 'hot' if self.view_mode == 'organ' else 'gray'
            ...
            if self.show_oblique_in == 'fourth':
                if self.view_mode == 'organ':
                    ...
                    self.ax_4th.set_title(
                        f'Organ - {self.organ_view_axis.capitalize()} ({view_descriptions[self.organ_view_axis]})', 
                        fontsize=10, fontweight='bold'
                    )
                else:
                    self.ax_4th.set_title(
    ...
    def draw_oblique_indicators(self):
        """Draw oblique plane indicators as yellow lines showing intersection - ENHANCED"""
    ...
    def update_info_panel(self):
        ...
        if self.view_mode == 'organ':
            ...
            mode_text = f"| Organ: {view_names[self.organ_view_axis]} View "
    ...
    def export_dicom(self, event):
        """Export DICOM with orientation selection popup"""
        ...
        for i in range(num_slices):
            ...
            if orientation == 'axial':
                ...
                ds.ImagePositionPatient = [0.0, float(i), 0.0]
            ...
            if i == 0:
                print(f"\n✓ First file ImageOrientationPatient: {ds.ImageOrientationPatient}")
    ...
    def show(self):
        ...
        print("  ✓ Orientation Detection (3D CNN)")
